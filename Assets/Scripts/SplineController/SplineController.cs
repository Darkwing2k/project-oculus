...
public enum eOrientationMode { NONE = 0, NODE, TANGENT, SPLINE, FIXED }
...
public class SplineController : MonoBehaviour
{
	...
	void SetupSplineInterpolator(SplineInterpolator interp, Transform[] trans) {
		...
		for (c = 0; c < trans.Length; ++c) {
			// if (OrientationMode == NODE) or (OrientationMode == TANGENT and is last Position without AutoClose)
			Quaternion rot = trans[c].rotation;

			if (OrientationMode == eOrientationMode.TANGENT) {
				if (c != trans.Length - 1)
					rot = Quaternion.LookRotation(trans[c + 1].position - trans[c].position, Vector3.up);
				else if (AutoClose)
					rot = Quaternion.LookRotation(trans[0].position - trans[c].position, Vector3.up);
			}
		...
		if (mTransforms.Length > 0) {
			...
			switch (OrientationMode) {
				...
				case eOrientationMode.NODE:
					...
					break;
				case eOrientationMode.FIXED:
					mSplineInterp.SetLookAt(lookAt);
					mSplineInterp.StartInterpolation(null, false, WrapMode);
					break;
			}
