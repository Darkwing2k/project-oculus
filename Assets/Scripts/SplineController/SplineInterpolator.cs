...
public enum OrientationMode { NONE, EXPLICIT, SPLINE, FIXED }
...
public class SplineInterpolator : MonoBehaviour
{
	...
	OrientationMode orientationMode = OrientationMode.NONE;
	...
	public void StartInterpolation(OnEndCallback endCallback, bool bRotations, eWrapMode mode)
	{
		...
		mState = mode == eWrapMode.ONCE ? State.Once : State.Loop;
	...
	public void SetLookAt(Transform lookAt) {
		...
		orientationMode = OrientationMode.FIXED;
	}
	...
	void Update()
	{
		...
		if (mState != State.Stopped)
		{
			...
			Vector3 nextPosition = GetHermiteInternal(mCurrentIdx, param);

			switch (orientationMode) {
				case OrientationMode.NONE:
				case OrientationMode.EXPLICIT:
					transform.rotation = GetSquad(mCurrentIdx, param);
					break;
				case OrientationMode.SPLINE:
